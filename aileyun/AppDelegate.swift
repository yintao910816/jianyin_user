...
//        })
        ...
        let not = Notification.init(name: NSNotification.Name.init(DID_BECOME_ACTIVE), object: nil, userInfo: nil)
...
extension AppDelegate {
    func application(_ app: UIApplication, open url: URL, options: [UIApplication.OpenURLOptionsKey : Any] = [:]) -> Bool {
        ...
        if url.scheme?.contains(kScheme) == true  {
            let not = Notification.init(name: NSNotification.Name.init(WCHAT_FINISH), object: nil, userInfo: nil)
            NotificationCenter.default.post(not)
        }
