...
//    }
    ...
    func HC_login(uname : String, code : String, callback : @escaping (Bool, String)->()){
        let dic = NSDictionary.init(dictionary: ["phone" : uname, "code" : code])
    ...
    //获取语音验证码
    func HC_validateCodeYY(phone : String, callback : @escaping (Bool, String)->()){
        let dic = NSDictionary.init(dictionary: ["phone" : phone])
        HttpClient.shareIntance.GET(HC_VALIDATECODE_YY, parameters: dic) { (result, ccb) in
            if ccb.success(){
                callback(true, ccb.msg)
            }else{
                callback(false, ccb.msg)
...
//    }
    ...
    func HC_findLastestTopics(pageNum: Int = 0, pageSize: Int = 10, callback : @escaping (Bool, [HCCircleModel]?, String)->()){
        let dic = NSDictionary.init(dictionary: ["sortId" : "1", "pageNum": pageNum, "pageSize": pageSize])
        HttpClient.shareIntance.GET(HC_LATEST_NEWS, parameters: dic, callBack: { (result, ccb) in
//            HCPrint(message: result)
            if ccb.success() {
                let dataDic = ccb.data as! [String : Any]
                
                let dicArr = dataDic["list"] as! [[String : Any]]
...
//                    FindRealClassForDicValue(dic: dic)
    ...
    func HC_notice(callback : @escaping ([NoticeHomeVModel]?, String)->()){
    ...
    func HC_jiaYinPay(price : String, type : String, preType : NSInteger, callback : @escaping(Bool, [String : Any]?)->()){
        let dic = NSDictionary.init(dictionary: ["price" : price, "type" : type, "preType" : preType])
        HttpClient.shareIntance.GET(HC_JIAYIN_PAY, parameters: dic) { (result, ccb) in
            HCPrint(message: result)
            if ccb.success(){
    ...
    func HC_jiayinPayResult(merOrderId : String, callback : @escaping(Bool)->()){
        let dic = NSDictionary.init(dictionary: ["merOrderId" : merOrderId])
        HttpClient.shareIntance.GET(HC_JIAYIN_PAY_RESULT, parameters: dic) { (result, ccb) in
            HCPrint(message: result)
            if ccb.success(){
                callback(true)
            }else{
    ...
    //实名认证
    func HC_bindID(phone : String, idNo : String, realName : String, callback : @escaping (Bool, String)->()){
        let dic = NSDictionary.init(dictionary: ["phone" : phone, "idNo" : idNo, "realName" : realName])
        HttpClient.shareIntance.GET(HC_CHECK_HOSPITAL_BIND, parameters: dic) { (result, ccb) in
            HCPrint(message: result)
            if ccb.success() {
                callback(true, ccb.msg)
                if let dic = ccb.data as? [String : Any]{
                    if let v = dic["visitCard"] as? String {
                        UserManager.shareIntance.HCUser?.visitCard = v
...
//    message = "\U67e5\U8be2\U6210\U529f";
    func HC_cardBalance(callback : @escaping (Bool, String)->()){
        HttpClient.shareIntance.GET(HC_JIAYIN_BALANCE, parameters: nil) { (result, ccb) in
//            HCPrint(message: result)
            if ccb.success() {
                if let dic = ccb.data as? [String : Any]{
                    if let balance = dic["balance"] as? String{
    ...
    func HC_rechargeType(cardNo : String, callback : @escaping (Bool, String)->()){
        let dic = NSDictionary.init(dictionary: ["cardNo" : cardNo])
        HttpClient.shareIntance.GET(HC_RECHARGE_TYPE, parameters: dic) { (result, ccb) in
            HCPrint(message: result)
            if ccb.success() {
                if let dic = ccb.data as? [String : Any]{
    ...
    func HC_unbindJIAYIN(callback : @escaping (Bool, String)->()){
        HttpClient.shareIntance.GET(HC_UNBIND_JIAYIN, parameters: nil) { (result, ccb) in
            HCPrint(message: result)
            if ccb.success() {
                callback(true, ccb.msg)
            }else{
    ...
    func HC_getBaiduAccessToken(callback : @escaping (Bool, String)->()) {
        HttpClient.shareIntance.GET(ACCESS_TOKEN, parameters: nil) { (result, ccb) in
            HCPrint(message: result)
            if ccb.success() {
                if let accessToken = ccb.data as? String {
                    callback(true, accessToken)
                }else {
                    callback(false, "token获取失败")
                }
            }else{
                callback(false, ccb.msg)
            }
    ...
    func baidubce(access_token: String, image: String, id_card_number: String, name: String, callBack:  @escaping (Bool, String)->()){
        HttpClient.shareIntance.BCE_POST(BAIDU_BCE,
                                         access_token: access_token,
                                         image: image,
                                         id_card_number: id_card_number,
                                         name: name) { (result, ccb) in
                                            HCPrint(message: result)
                                            if ccb.success(), let score = ccb.data as? Float {
                                                HCPrint(message: "公安验证分数：\(score)")
                                                callBack(true, "\(score)")
                                            }else{
                                                callBack(false, ccb.msg)
                                            }
