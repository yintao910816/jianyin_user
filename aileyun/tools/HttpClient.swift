...
extension HttpClient {
    ...
    func BCE_POST(_ URLString : String, access_token : String,
                  image: String,
                  id_card_number: String,
                  name: String,
                  callBack : @escaping HttpRequestCompleted){
        let url = URLString + "?access_token=\(access_token)"
        let params = ["image": image,  "image_type":"BASE64", "id_card_number": id_card_number, "name": name, "liveness_control": "NORMAL"]
        HCmanager.post(url, parameters: params, progress: { (progress) in
            //
        }, success: { (task : URLSessionDataTask, responseObject : Any) in
            ...
            guard let scoreInfo = dic.value(forKey: "result") as? NSDictionary else {
                ccb.code = 10000
                ccb.msg = "json解析失败"
                callBack("", ccb)
                return
            ...
            guard let score = scoreInfo.value(forKey: "score") as? Float else {
                ccb.code = 10000
                ccb.msg = "json解析失败"
                callBack("", ccb)
                return
...
//        dic["token"] = "QW5peHdiSnJNWWFIUi9aTHRJQ0x0YnVHaHdDM1dHaXNvdExTNDAzUVoxTlJSNVNJcm5OUk13PT0="
        dic["deviceType"] = "iOS"
